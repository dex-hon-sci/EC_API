...
class RingTickBuffer(TickBuffer):
    ...
    def get_window(
        self, horizon: float | None = None, current_time: float | None = None
        ...
        if self.full:
            ...
            prices, vols, ts = (
                self.prices[: self.cur],
                self.volumes[: self.cur],
                self.timestamps[: self.cur],
            )
...
class TimeTickBuffer(TickBuffer):
    ...
    @time_it
    def _pop_old_ticks(self, buf, timeframe: float, current_time: float) -> None:
