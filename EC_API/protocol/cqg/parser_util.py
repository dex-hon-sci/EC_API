...
def walk_fields(
    msg: ServerMsg, selector: Callable[[Any, Any], Iterable[Any]], max_depth: int = 2
) -> list[Any]:
    outs: list[Any] = []
...
def parse_server_msg(
    server_msg: ServerMsg, parsers: dict[str, Parser_func]
) -> list[dict[str, Any]]:
    ...
    for msg_type in msg_types:
        ...
        if parser is None:
            errors.append(f"Parser of message type: {msg_type} does not exist.")
            continue
        try:
            res.extend(parser(server_msg))
        except Exception as e:
            errors.append(f"Failed to parse server message: {e}.")
            # continue
