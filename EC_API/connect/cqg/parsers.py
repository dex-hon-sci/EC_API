...
def parse_pong(server_msg: ServerMsg) -> tuple[str, str, int, int]:
    try:
        return (
            "pong",
            server_msg.pong.token,
            server_msg.pong.ping_utc_time,
...
def parse_symbol_resolution_report(report: InformationReport) -> dict[str, str]:
    try:
        return MessageToDict(report.symbol_resolution_report, preserving_proto_field_name=True)
    except Exception:
        raise MsgParserError("Failed to parse symbol_resolution_report.")


@register_info_report_parsers("session_information_report")
def parse_session_information_report(report: InformationReport) -> dict[str, str]:
    try:
        return MessageToDict(report.session_information_report, preserving_proto_field_name=True)
    except Exception:
        raise MsgParserError("Failed to parse session_information_report.")


@register_info_report_parsers("historical_orders_report")
def parse_historical_orders_report(report: InformationReport) -> list[dict[str, str]]:
    try:
        return MessageToDict(report.historical_orders_report, preserving_proto_field_name=True)
    except Exception:
        raise MsgParserError("Failed to parse historical_orders_report.")


@register_info_report_parsers("option_maturity_list_report")
def parse_option_maturity_list_report(report: InformationReport) -> list[dict[str, str]]:
    try:
        return MessageToDict(report.option_maturity_list_report, preserving_proto_field_name=True)
    except Exception:
        raise MsgParserError("Failed to parse option_maturity_list_report.")


@register_info_report_parsers("instrument_group_report")
def parse_instrument_group_report(report: InformationReport) -> list[dict[str, str]]:
    try:
        return MessageToDict(report.instrument_group_report, preserving_proto_field_name=True)
    except Exception:
        raise MsgParserError("Failed to parse instrument_group_report.")


@register_info_report_parsers("at_the_money_strike_report")
def parse_at_the_money_strike_report(report: InformationReport) -> list[dict[str, str]]:
    try:
        return MessageToDict(report.at_the_money_strike_report, preserving_proto_field_name=True)
    except Exception:
