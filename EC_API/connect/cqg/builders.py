...
from EC_API.protocol.cqg.builder_util import assert_input_types
...
def build_pong_msg(token: str, ping_utc_time: int, pong_utc_time: int) -> ClientMsg:
    params = locals().copy()
    try:
...
def build_resolve_symbol_msg(
    symbol_name: str, request_id: int, subscribe: bool | None = None, **kwargs
) -> ClientMsg:
    kwargs = dict({}, **kwargs)
    params = locals().copy()
    params.pop("kwargs")
    ...
    if "instrument_group_request" in kwargs:
        information_request.instrument_group_request.instrument_group_id = kwargs[
