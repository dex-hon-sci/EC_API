...
class Payload:
    ...
    start_time: datetime = datetime.now(timezone.utc) + timedelta(days=1)  # In long text format
    end_time: datetime = datetime.now(timezone.utc) + timedelta(days=2)  # In long text format
        ...
        if payload.status == PayloadStatus.PENDING:
            try:
                await self.live_order.send(
                    request_type=payload.order_request_type, request_details=payload.order_info
                )
            ...
            logger.warning("Invalid Payload Status: Only pending payloads can be unloaded.")
