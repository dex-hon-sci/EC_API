...
from EC_API._typing import MarketValueTypeCQG, QuotesValueTypeCQG, DOMValueTypeCQG, ParsedRTMDCQG
...
def parse_market_data_subscription_statuses(server_msg: ServerMsg) -> list[dict[str, str]]:
    try:
        market_data_subscription_statuses = server_msg.market_data_subscription_statuses
        return [
            {"contract_id": msg.contract_id, "status_code": msg.status_code, "level": msg.level}
...
def _parse_quotes(real_time_market_data: RealTimeMarketData) -> list[QuotesValueTypeCQG]:
    ...
    mkt_state = (
        _parse_market_state(real_time_market_data.market_state)
        if real_time_market_data.HasField("market_state")
    ...
    for ele in real_time_market_data.quotes:
        if ele.HasField("trade_attributes"):
            ...
            ta_agreement_time_utc = (
                ta.agreement_time_utc.seconds + ta.agreement_time_utc.nanos / 1e9
            )
        else:
...
def _parse_market_values(real_time_market_data: RealTimeMarketData) -> list[MarketValueTypeCQG]:
    ...
    for ele in real_time_market_data.market_values:
        ...
        last_trade_utc_timestamp = (
            ele.last_trade_utc_timestamp.seconds + ele.last_trade_utc_timestamp.nanos / 1e9
        )
...
def parse_real_time_market_data(
    ...
    try:
        ...
        quotes: list[QuotesValueTypeCQG] = (
            _parse_quotes(real_time_market_data) if real_time_market_data.quotes else []
        )
        mkt_vals: list[MarketValueTypeCQG] = (
            _parse_market_values(real_time_market_data)
            if real_time_market_data.market_values
            else []
        )
        dtl_DOMs: list[DOMValueTypeCQG] = []  # fix later
        corrections: list[QuotesValueTypeCQG] = (
            _parse_quotes(real_time_market_data) if real_time_market_data.corrections else []
        )
