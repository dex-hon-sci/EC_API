...
def validate_required_fields(params: dict[str, Any], required_fields: dict[str, Any]) -> None:
...
def validate_input_para(order_info: dict[str, Any]) -> None:
    ...
    if order_info.get("qty_exponent") is not None:
        ...
        if order_info["qty_exponent"] < lower_limit or order_info["qty_exponent"] > upper_limit:
            raise ValueError(
                f"qty_exponent is outside of the range [{lower_limit}, {upper_limit}]."
            )
