...
class OrderStatusCQG(Enum):
    ...
    IN_TRANSIT = (
        auto()
    )  # = OrderStatus.Status.IN_TRANSIT  # Original order is sent to execution system.
    ...
    SUSPENDED = (
        auto()
    )  # OrderStatus.Status.SUSPENDED # Order is waiting submission to execution system.
    DISCONNECTED = (
        auto()
    )  # OrderStatus.Status.DISCONNECTED # Order may be canceled because a disconnect occurred.
